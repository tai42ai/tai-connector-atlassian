...
`tai_app.connectors.register_connector(descriptor)` when the manifest loads it.
...
It depends on **`tai-contract` only**.
...
for Google (`tai-connector-google`). The ecosystem is open-ended: any package
...
`src/tai_connector/atlassian/core/connector.py` builds the descriptor and
registers it on import. `tai_connector` is a namespace package, so each provider
...
uv add --editable ../tai-connector-atlassian   # once published: uv add tai-connector-atlassian
```

## Development

```bash
uv sync                  # public: tai-contract + tooling
uv run ruff check .
uv run pyright
