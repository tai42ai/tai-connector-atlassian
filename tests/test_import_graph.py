...
``tai42_connector.atlassian`` is on the allowlist. The rule (see the README): the shipped
package imports
...
The runtime walk imports ``tai42_connector.atlassian`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_connector.atlassian`` means the assertion covers the SHIPPED package's
...
# Every third-party root the shipped ``tai42_connector.atlassian`` graph pulls in -- the declared runtime dependencies
