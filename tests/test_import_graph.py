...
``tai_connector.atlassian`` is on the allowlist. The rule (see the README): the shipped
package imports
the shared platform contract (tai-contract) and its dependency closure ONLY --
it reaches Atlassian over the host's HTTP seam and ships no vendor SDK --
...
The runtime walk imports ``tai_connector.atlassian`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_connector.atlassian`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_connector.atlassian"
ALLOWED_FIRST_PARTY = frozenset({"tai_connector", "tai_contract"})

# Every third-party root the shipped ``tai_connector.atlassian`` graph pulls in -- the declared runtime dependencies
# plus their resolved closure. Adding a runtime dependency that brings a new root means adding that root here, but
# only when it is a genuine dependency of the shipped package -- the walks below are never widened just to make the
# test pass.
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_connector.atlassian graph failed:\n"
        f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_connector.atlassian module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_connector.atlassian sources: {offenders}"
